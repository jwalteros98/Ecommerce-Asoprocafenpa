...
export class DescuentoService {
  ...
  listar_descuento_admin(filtro:any, token:any):Observable<any>{  //any, no esta en el curso
    let headers = new HttpHeaders({ 'Content-Type' : 'application/json', 'authorization' : token });
    return this._http.get(this.url+'listar_descuento_admin/' + filtro, {headers: headers});
  }
  eliminar_descuento_admin(id:any, token:any):Observable<any>{  //any, no esta en el curso
    let headers = new HttpHeaders({ 'Content-Type' : 'application/json', 'authorization' : token });
    return this._http.delete(this.url+'eliminar_descuento_admin/' + id, {headers: headers});
  }

  registro_descuento_admin(data: any, file: any, token:any):Observable<any>{  // any, no esta en el curso
  ...
  obtener_descuento_admin(id:any, token:any):Observable<any>{  //any, no esta en el curso
    let headers = new HttpHeaders({ 'Content-Type' : 'application/json', 'authorization' : token });
    return this._http.get(this.url+'obtener_descuento_admin/' + id, {headers: headers});
  }

  actualizar_descuento_admin(data: any, id: any, token:any):Observable<any>{  // any, no esta en el curso
    if (data.banner) {
      let headers = new HttpHeaders ({'Authorization': token});
